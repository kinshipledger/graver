...
| SEC-004 | An unexpectedly large provider response could consume excessive memory | Medium | Fixed: the Requests transport streams into a bounded 8 MiB buffer, rejects an oversized declared or observed body with a typed error, closes the response, and persists nothing. Covered by offline boundary tests; tracked in [issue 80](https://github.com/mcqueary/graver/issues/80). |
